...
### Fixed
- **Group 1 Credit & Watermark Filter (`ocr.py`)**: Implemented pre-translation noise and credit filtering (`is_noise_credit_or_watermark`) to filter out known platform watermarks (`本章节仅限...`), scanlation recruitment end-pages (`QQ群...`), and serial license codes (`皖A·...`). Reduces token usage by another 10-15% while guaranteeing 0% narrative dialogue loss. Added comprehensive tests in `test_credit_watermark_filter.py`.
- **Bubble Inpainting Bottom Padding (`inpainter.py`)**: Expanded bottom vertical padding on `clean_box` (`max(10, int((y2 - y1) * 0.20))`) to ensure multi-line stylized exclamation or drop-cap text (`VOICE!!`) below the primary bubble line is completely whitened out before typesetting Thai text.
- **Usable Thai Translation Acceptance & Soft QC Flags (`worker.py`)**: Resolved issue where soft advisory QC flags (such as `NUMBER_MISMATCH` when English numbers are written as words like "THIRTY-FIVE" while Thai uses digits "35") previously caused valid Thai translations to be discarded and reverted to English source text. Now soft QC flags (`_SOFT_QC_ISSUES`) are accepted into `approved` translations, ensuring dialogue bubbles render properly in Thai while un-translated SFX/symbols retain their original text.
- **Dynamic DeepSeek Recovery Output Tokens (`worker.py`)**: Scaled `max_output_tokens` dynamically (`max(settings.DEEPSEEK_RECOVERY_MAX_OUTPUT_TOKENS, len(segments) * 200)`) for adaptive recovery batches to prevent output truncation and `Zero outcome` on large segment batches.
- **Resilience Publication Guard (`worker.py`)**: Resolved complete reader lockout ("หนักจนกระทั่งเข้าอ่านไม่ได้เลย") where a single unapproved SFX or budget exhaustion caused the entire chapter to fail and withhold all pages. Chapters now publish with status `COMPLETED_WITH_WARNINGS` as long as at least one dialogue segment succeeds (`total_approved_count > 0`), ensuring users can immediately read completed manga pages.
- **Concurrent Stage 2 Batch Translation & Stage 4 R2 Uploads (`worker.py`)**: Resolved slow translation speed ("แปลช้ามาก") by replacing serial Stage 2 batch translation with bounded concurrent execution via `asyncio.Semaphore(batch_limit)` + `asyncio.gather`, and replacing serial Stage 4 R2 image uploads with concurrent async tasks via `asyncio.gather`.
- **Multi-Angle Shear OCR for Slanted/Italic Text (`ocr.py`)**: Enhanced ROI slanted text detection ("แปลหน้าตัวอักษรเอียงไม่ได้") by sweeping multiple shear angles `(-0.20, -0.12, +0.12)` during upscaled affine crop recovery. Also added unit tests in `test_slanted_ocr.py`.
- Added typed `DeepSeekBatchParseOutcome` (`outcome_type`: `OK`, `EMPTY_CONTENT`, `MALFORMED_JSON`, `PARTIAL`, `ZERO_OUTCOME`) and adaptive batch splitting (`_adaptive_translate_segments`) in `TranslationPipelineWorker` Stage 2 to recursively divide batches when partial or empty outcomes occur without sleep backoff or crashing. Added comprehensive integration tests in `test_translation_reliability_recovery.py`.
- Enforced forward slashes (`/`) and standard command names (`pytest`, `python -m pytest`) in `AGENTS.md` and `RULES.md` across all agents to eliminate Windows backslash (`\`) regex escape bugs in permission matchers that caused interactive approval prompts on every command run.
...
- Replaced the active `plan.md` with an evidence-backed translation reliability recovery plan. The failing Chapter 149 job had 76 accepted translations and 13 genuinely missing IDs, while 20 page-level OCR diagnostics were incorrectly counted as dialogue failures. The new plan freezes all bubble geometry, introduces typed DeepSeek outcomes, region-keyed translation state, hard/soft QC, adaptive provider-local recovery, checkpoints, paid-API budgets, and exact region snapshot regressions.
